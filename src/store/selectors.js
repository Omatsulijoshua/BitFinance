...
export const myOpenOrdersSelector = createSelector(
    ...
    (account, tokens, orders) => {
      ...
      orders = orders.filter((o) => o.tokenGet === tokens[0].target || o.tokenGet === tokens[1].target)
      orders = orders.filter((o) => o.tokenGive === tokens[0].target || o.tokenGive === tokens[1].target)
...
const decorateMyOpenOrder = (order, tokens) => {
  let orderType = order.tokenGive === tokens[1].target ? 'buy' : 'sell'
...
const decorateOrder = (order, tokens) => {
  ...
  if (order.tokenGive === tokens[1].target) {
  ...
  let tokenPrice = 0
  if (token0Amount && token0Amount.toString() !== '0') {
    const token0Units = parseFloat(ethers.formatUnits(token0Amount, "ether"))
    const token1Units = parseFloat(ethers.formatUnits(token1Amount, "ether"))
    tokenPrice = token1Units / token0Units
  }
...
const decorateFilledOrders = (orders, tokens) => {
  ...
  return(
    orders.map((order) => {
      ...
      order = decorateOrder(order, tokens)
      order = decorateFilledOrder(order, previousOrder, tokens)
...
const decorateFilledOrder = (order, previousOrder, tokens) => {
  const orderType = order.tokenGive === tokens[1].target ? 'buy' : 'sell'
  return({
    ...order,
    orderType,
    orderTypeClass: (orderType === 'buy' ? GREEN : RED),
...
const tokenPriceClass = (tokenPrice, orderId, previousOrder) => {
  ...
  if (previousOrder && previousOrder.id && orderId && previousOrder.id.toString() === orderId.toString()) {
    return GREEN
  }
...
const decorateMyFilledOrder = (order, account, tokens) => {
  ...
  let orderType
  if(myOrder) {
    orderType = order.tokenGive === tokens[1].target ? 'buy' : 'sell'
  } else {
    orderType = order.tokenGive === tokens[1].target ? 'sell' : 'buy'
  }
